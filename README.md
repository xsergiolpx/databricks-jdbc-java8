...
This guide documents the exact steps used so KBTG can replicate the setup end to end.
...
Compile with Java 17 (targeting Java 8 bytecode).

Run with the Java 8 runtime to validate compatibility.
...
ls /usr/lib/jvm/
Expected example path: /usr/lib/jvm/java-1.8.0-amazon-corretto.x86_64/jre/bin/java
...
Ensure Class.forName("com.databricks.client.jdbc.Driver") is present in the code.

Ensure the full runtime classpath is included (dependency:build-classpath).
